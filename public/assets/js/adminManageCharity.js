function viewCharity(id){
    fetch(`http://localhost/SurplusStays/public/admin/charityDetails/${id}`)
    .then(response => {
        if(!response.ok) {
    ...
    .then(data => {
        window.location.href=`http://localhost/SurplusStays/public/admin/viewCharity/${data[0].org_id}`
    })
